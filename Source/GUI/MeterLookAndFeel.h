...
public:
    ...
    void drawMeters(juce::Graphics &g, const juce::Rectangle<float> &bounds,
        ...
        numberBound = numberBound.withTrimmedTop(0.05f * meterWidth);
        numberBound = numberBound.withTrimmedBottom(0.05f * meterWidth);
        numberBound = numberBound.withTrimmedTop(0.08f * numberBound.getHeight());
        drawMeterNumbers(g, numberBound);
        for (size_t i = 0; i < peakMax.size(); ++i) {
            auto localBound = bound.removeFromLeft(meterWidth);
            localBound = localBound.withSizeKeepingCentre(0.9f * localBound.getWidth(),
...
private:
    ...
    void drawMeterPeakMax(juce::Graphics &g, float peakMax, const juce::Rectangle<float> &bound) {
        ...
        if (peakMax < minRMS) {
            ...
            peakMaxDisplay = ZLInterface::formatFloat(peakMax, 1);
        }
    ...
    void drawMeterNumbers(juce::Graphics &g, const juce::Rectangle<float> &bound) {
        ...
        for (int i = 0; i < numNumbers; ++i) {
            ...
            g.drawSingleLineText("-" + ZLInterface::formatFloat(std::abs(localNumber), 0) + "-",
